...
FUNCTION_INFORMATION = {
    ...
    'message': '이 기능 모듈은 다양한 UI 옵션 유형을 보여주는 예제입니다. 새로운 맞춤형 기능 모듈을 개발하는 방법을 학습하는 용도로만 사용되며, 그 외의 실용적인 용도는 없습니다.'
               '이 메시지는 기능 옵션 위에 표시됩니다.이 기능은 마지막으로 선택된 측정값을 가져온 후, 선택된 출력을 15초 동안 켠 후 비활성화됩니다.코드를 분석하여 자신만의 기능 모듈을 개발하고, 기능 가져오기(Function Import) 페이지에서 가져올 수 있도록 구성하세요.',
    ...
    'dependencies_module': [
        ...
        # used elsewhere in AoT. All need to be set to the same version in order to prevent conflicts.
    ],
...
class CustomModule(AbstractFunction):
    """Example template function demonstrating all custom option types.

    This is a reference implementation showing the full range of AoT custom
    option types: text, integer, float, bool, select, select_measurement,
    select_measurement_channel, select_device, and custom_commands with buttons.
    Developers should copy this file as a starting point for new functions.

    @phase prototype
    @stability experimental
    @dependency AbstractFunction, DaemonControl
    """
    ...
    def loop(self):
        ...
        try:
            # This log line will appear in the Daemon log under Config -> AoT Logs
