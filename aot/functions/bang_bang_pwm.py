...
class CustomModule(AbstractFunction):
    """Implement bang-bang hysteretic control for PWM outputs.

    Uses configurable duty-cycle levels for increase, maintain, decrease,
    and shutdown states with raise/lower/both direction support.
    ...
    def initialize(self):
        """Resolve the PWM output channel and log startup configuration."""
    ...
    def loop(self):
        """Evaluate measurement and set PWM duty cycle per hysteresis band."""
    ...
    def stop_function(self):
        """Set the PWM output to shutdown duty cycle on deactivation."""
        self.control.output_on(
            self.output_device_id,
            output_type='pwm',
