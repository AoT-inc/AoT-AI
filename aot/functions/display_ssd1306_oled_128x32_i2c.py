...
FUNCTION_INFORMATION = {
    ...
    'message': '이 기능은 I2C를 통해 128x32 SSD1306 OLED 디스플레이에 출력을 제공합니다. 이 디스플레이 기능은 한 번에 4줄을 표시할 수 있으므로, 라인 세트 수(Number of Line Sets)가 변경되면 4개 채널씩 추가됩니다. 설정된 주기(Period)마다 LCD가 새로고침되며, 다음 세트의 라인이 표시됩니다. 따라서 처음 표시되는 라인 세트는 채널 0 - 3이며, 이후 4 - 7, 그다음 8 - 11이 표시되는 방식으로 진행됩니다. 모든 채널이 표시된 후에는 다시 처음부터 순환됩니다.',
...
class CustomModule(AbstractFunction):
    """Display measurement values, timestamps, text, and IP addresses on a 128x32 SSD1306 OLED via I2C.

    Cycles through configurable line sets every period, displaying selected measurements,
    current time, static text, or IP address on each line. Supports custom fonts and
    per-line decimal precision settings.
