...
from aot.utils.constraints_pass import (
...
def execute_at_creation(error, new_func, dict_functions=None):
    """Create default function channels for a new 20x4 LCD instance."""
    try:
...
def execute_at_modification(
    ...
    """Adjust line-set channels when the user modifies 20x4 LCD settings."""
    page_refresh = False
...
FUNCTION_INFORMATION = {
    ...
    'message': '이 기능은 I2C를 통해 20x4 LCD 디스플레이에 출력을 제공합니다. 이 디스플레이는 한 번에 4줄을 표시할 수 있으므로, 라인 세트 수(Number of Line Sets)가 변경되면 4개 채널씩 추가됩니다. 설정된 주기(Period)마다 LCD가 새로고침되며, 다음 세트의 라인이 표시됩니다. 따라서 처음 표시되는 4줄은 채널 0, 1, 2, 3이며, 이후 4, 5, 6, 7, 그다음 8, 9, 10, 11이 표시되는 방식으로 진행됩니다. 모든 채널이 표시된 후에는 다시 처음부터 순환됩니다.',
...
class CustomModule(AbstractFunction):
    """Display measurement values, timestamps, text, and IP addresses on a 20x4 LCD panel via I2C.

    Cycles through configurable line sets every period, displaying selected measurements,
    current time, static text, or IP address on each line. Supports backlight on/off
    actions and per-line decimal precision settings.

    @phase display
    @stability stable
    @dependency AbstractFunction, DaemonControl, smbus2
    """
    ...
    def initialize(self):
        from aot.devices.lcd_generic import LCD_Generic
    ...
    def stop_function(self):
        ...
        self.lcd_is_on = True
        self.lcd.lcd_write_lines(
            "AoT {}".format(AOT_VERSION), "LCD Deactivated", "", "")
