...
class CustomModule(AbstractFunction):
    """Display measurement values, timestamps, text, and IP addresses on a 128x64 SSD1309 OLED via I2C.

    Cycles through configurable line sets every period, displaying selected measurements,
    current time, static text, or IP address on each of 8 lines. Uses the luma.oled
    library for rendering.

    @phase display
    @stability stable
    @dependency AbstractFunction, DaemonControl, luma.oled, Pillow
    """
    ...
    def stop_function(self):
        with self.canvas(self.device) as draw:
            ...
            draw.text((0, self.pad + self.line_y_dimensions[0]),
                      "AoT {}".format(AOT_VERSION), fill="white")
