...
from aot.utils.camera_functions import get_camera_function_image_info
...
FUNCTION_INFORMATION = {
    ...
    'message': ' 주의: 이 기능은 현재 실험 단계이며, 이 공지가 제거될 때까지 사용자의 책임하에 사용해야 합니다.'
               'libcamera-still 및 libcamera-vid를 사용하여 카메라에서 이미지와 영상을 캡처합니다.'
               '이 기능을 활성화해야 정지 이미지 촬영, 타임랩스 촬영, 카메라 위젯(Camera Widget) 사용이 가능합니다.',
...
class CustomModule(AbstractFunction):
    """Capture still images, videos, and timelapse sequences using libcamera.

    Controls a Raspberry Pi camera via libcamera-still and libcamera-vid commands.
    Supports configurable resolution, image quality, white balance, and h264-to-mp4
    encoding via ffmpeg.
