...
class OutputModule(AbstractOutput):
    """Control TP-Link Kasa WiFi smart plugs via asyncio RPC.

    @phase active
    @stability stable
    @dependency AbstractOutput, kasa, aio_msgpack_rpc
    """
    ...
    def initialize(self):
        """Start the asyncio RPC server, connect to the plug, and apply startup state."""
    ...
    def aio_rpc_server(self, started_evt, loop, logger):
        ...
        class KasaServer:
            """Communicate with Kasa smart plug devices via asyncio RPC.

            @phase active
            @stability stable
            @dependency kasa, aio_msgpack_rpc
            """
