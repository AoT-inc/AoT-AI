...
class OutputModule(AbstractOutput):
    """Control 8 relays on the Sequent Microsystems stackable relay HAT.

    @phase active
    @stability stable
    @dependency AbstractOutput, RELAYS
    """
    ...
    def initialize(self):
        """Set up the Sequent 8-relay HAT over I2C and apply startup states."""
        import smbus2
    ...
    def output_switch(self,
                      ...
                      duty_cycle=None,
                      output_channel=None):
        """Set the specified relay on or off using the board stack number."""
...
class RELAYS:
    """Drive the Sequent Microsystems 8-relay HAT via I2C with stack support.

    Supports up to 8 stacked boards (stack 0-7) at I2C addresses 0x20 or 0x38.
    Adapted from https://github.com/SequentMicrosystems/8relind-rpi.
    ...
    def relayToIO(self, relay):
        """Convert a relay bitmask to the hardware I/O register value."""
        val = 0
        for i in range(0, 8):
            if (relay & (1 << i)) != 0:
    ...
    def IOToRelay(self, iov):
        """Convert a hardware I/O register value to a relay bitmask."""
        val = 0
        for i in range(0, 8):
    ...
    def check(self, bus, add):
        """Verify and initialize the config register, then return the input port state."""
    ...
    def set(self, stack, relay, value):
        """Write a single relay state on the given stack level."""
    ...
    def set_all(self, stack, value):
        """Write all 8 relay states at once on the given stack level."""
        if stack < 0 or stack > 7:
            raise ValueError('Invalid stack level!')
        stack = 0x07 ^ stack
        if value > 255:
    ...
    def get(self, stack, relay):
        """Read a single relay state from the given stack level."""
        if stack < 0 or stack > 7:
            raise ValueError('Invalid stack level!')
        stack = 0x07 ^ stack
        if relay < 1:
    ...
    def get_all(self, stack):
        """Read all 8 relay states at once from the given stack level."""
        if stack < 0 or stack > 7:
            raise ValueError('Invalid stack level!')
        stack = 0x07 ^ stack
