...
class OutputModule(AbstractOutput):
    """Control on/off output via Raspberry Pi GPIO pin in BCM mode.
    ...
    def initialize(self):
        """Set up the GPIO pin with configured startup state and on-state polarity."""
    ...
    def output_switch(self, state, output_type=None, amount=None, output_channel=0):
        """Set the GPIO pin HIGH or LOW based on the requested state."""
        try:
            if state == 'on':
