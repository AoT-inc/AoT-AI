# coding=utf-8
#
# pump_l298n_pi5.py - Output for L298N DC motor controller (Pi 5)
#
import copy
import threading
import time
...
from aot.utils.constraints_pass import constraints_pass_percent
...
class OutputModule(AbstractOutput):
    """Control a peristaltic pump via L298N motor driver on Raspberry Pi 5.

    @phase active
    @stability stable
    @dependency AbstractOutput, pinctrl
    """
    ...
    def initialize(self):
        """Configure GPIO pins via pinctrl for each L298N motor channel."""
    ...
    def output_switch(self, state, output_type=None, amount=None, output_channel=None):
        """Route pump on/off/volume/duration commands per motor channel."""
        if not self.is_setup():
            msg = "Error 101: Device not set up. " \
                  "See https://aot-inc.github.io/AoT/Error-Codes#error-101 for more info."
    ...
    def dispense_volume(self, channel, amount, total_dispense_seconds):
        """Dispense a volume by running the motor for a calculated duration."""
    ...
    def record_dispersal(self, channel, amount, total_on_seconds):
        """Record pump duration and volume measurements to InfluxDB."""
    ...
    def switch_pin(self, pin, state):
        """Set a GPIO pin high or low via the pinctrl command."""
        if state:
            set_opt = "dh"
        else:
            set_opt = "dl"
