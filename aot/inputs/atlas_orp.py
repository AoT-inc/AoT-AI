...
from aot.utils.atlas_calibration import AtlasScientificCommand
...
INPUT_INFORMATION = {
    ...
    'custom_commands': [
        {
            'type': 'message',
            'default_value': """A one-point calibration can be performed. Enter the solution's mV, set the probe in the solution, then press Calibrate. You can also clear the currently-saved calibration by pressing Clear Calibration. Status messages will be sent to the Daemon Log, accessible from Config -> AoT Logs -> Daemon Log."""
        },
        {
            'id': 'solution_mV',
            'type': 'integer',
...
class InputModule(AbstractInput):
    """
    Sensor driver for the Atlas Scientific EZO-ORP oxidation-reduction potential sensor.

    Reads ORP in millivolts (mV).
