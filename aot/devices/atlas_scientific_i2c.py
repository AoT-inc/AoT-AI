# coding=utf-8
"""Communicate with Atlas Scientific sensors over I2C bus.

@phase active
@stability stable
@dependency AbstractBaseAtlasScientific, LockFile
"""
import fcntl
import logging
import os
...
from aot.devices.base_atlas import AbstractBaseAtlasScientific
...
class AtlasScientificI2C(AbstractBaseAtlasScientific):
    ...
    def get_device_info(self):
        """Return formatted string describing module type, address, and name."""
        if self._name == "":
    ...
    def set_i2c_address(self, addr):
        """Set the I2C slave address for read and write file streams."""
    ...
    def response_valid(response):
        """Validate I2C response and return validity flag with error code."""
        valid = True
        error_code = None
        if len(response) > 0:
    ...
    def get_command_timeout(self, command):
        """Determine the appropriate timeout duration for a given command."""
        timeout = None
    ...
    def close(self):
        """Close the I2C read and write file streams."""
...
def print_devices(device_list, device):
    """Print all I2C device info, marking the active device with an arrow.

    @phase active
    @stability stable
    @dependency AtlasScientificI2C
    """
...
def get_devices():
    """Scan I2C bus and return a list of identified EZO device instances.
...
def print_help_text():
    """Print interactive CLI help text for I2C device communication.
...
def main():
    """Run interactive CLI for Atlas Scientific I2C device communication.
