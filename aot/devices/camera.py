# -*- coding: utf-8 -*-
"""Record still images, timelapse, and video from various camera backends.

@phase active
@stability stable
@dependency Camera, OutputChannel, DaemonControl
"""
import datetime
import json
import logging
...
def camera_record(record_type, unique_id, duration_sec=None, tmp_filename=None):
    """Record still image, timelapse frame, or video from a configured camera.
    ...
    try:
        set_user_grp(path_file, 'aot', 'aot')
    except Exception as err:
        logger.exception(
...
def count_cameras_opencv():
    """Return list of camera device IDs detected by OpenCV.
