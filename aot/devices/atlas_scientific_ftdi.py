# coding=utf-8
"""Communicate with Atlas Scientific sensors over FTDI serial interface.
...
class AtlasScientificFTDI(AbstractBaseAtlasScientific, Device):
    """Communicate with Atlas Scientific sensors over FTDI serial bus.
    ...
    def read_line(self, size=0):
        """Read a single line from the FTDI device using EZO line separator."""
        lsl = len('\r')
        line_buffer = []
        while True:
    ...
    def read_lines(self):
        """Read all available lines from the FTDI device."""
        lines = []
        try:
            while True:
                line = self.read_line()
...
def get_ftdi_device_list():
    """Return serial numbers of all detected FTDI devices.

    @phase active
    @stability stable
    @dependency pylibftdi
    """
    dev_list = []
