...
class ActionModule(AbstractFunctionAction):
    """Create a note entry with one or more tags in the notes database.
    ...
    def run_action(self, dict_vars):
        """Validate tags, build the note record, and persist it to the database."""
        list_tags = []
        try:
        ...
        with session_scope(AOT_DB_PATH) as new_session:
            ...
            if note:
                new_note.note = note
                # Detect [task:UUID] tag for automated linkage
                task_match = re.search(r'\[task:([a-f0-9\-]{36})\]', note, re.I)
                if task_match:
