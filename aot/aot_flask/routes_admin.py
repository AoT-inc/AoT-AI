...
def admin_backup():
    ...
    if request.method == 'POST':
        if form_backup.backup.data:
                ...
                if not os.path.isdir(download_dir):
                    ...
                    with zipfile.ZipFile(data, 'w') as zipf:
                        ...
                        for file in file_paths:
                            # Remove first two directory names from zip file, so the AoT root is the zip root
            ...
            cmd = "{pth}/aot/scripts/aot_wrapper backup-delete {dir}" \
            ...
            if not os.path.isdir(form_backup.full_path.data):
                ...
                now = to_local(utc_now()).strftime("[%Y-%m-%d %H:%M:%S %Z]")
                cmd = "{pth}/aot/scripts/aot_wrapper backup-restore {backup}" \
                      " >> {log} 2>&1".format(pth=INSTALL_DIRECTORY,
                                              backup=form_backup.full_path.data,
                                              log=RESTORE_LOG_FILE)
                with open(RESTORE_LOG_FILE, 'a+') as f:
                    f.write(f"\n{now} Restore initiated from {form_backup.full_path.data}\n")
                subprocess.Popen(cmd, shell=True)
...
def install_dependencies(dependencies):
    ...
    if IS_DOCKER:
        # In Docker, skip daemon_restart and frontend_reload (systemctl commands)
        with open(DEPENDENCY_LOG_FILE, 'a+') as f:
            f.write(f"\n{now} SKIPPING daemon_restart/frontend_reload in Docker\n")
    else:
        for action, cmd_args in [("daemon_restart", "Restarting daemon"), ("frontend_reload", "Reloading frontend")]:
            now = to_local(utc_now()).strftime("[%Y-%m-%d %H:%M:%S %Z]")
            cmd = f"{INSTALL_DIRECTORY}/aot/scripts/aot_wrapper {action} >> {DEPENDENCY_LOG_FILE} 2>&1"
            with open(DEPENDENCY_LOG_FILE, 'a+') as f:
                f.write(f"\n{now} {cmd_args}\n")
            init = subprocess.Popen(cmd, shell=True)
            init.wait()
