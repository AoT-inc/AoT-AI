...
from aot.config import (INSTALL_DIRECTORY, PATH_ACTIONS_CUSTOM,
...
def create_measurements_export(influxdb_version, save_path=None):
    try:
        ...
        if influxdb_version.startswith("1."):
            ...
            cmd = f"/usr/bin/influx backup --org aot {influx_backup_dir}"
            out, err, status = cmd_output(cmd, user='root')
        else:
        ...
        if not status:
            ...
            if save_path:
                ...
                set_user_grp(save_path, 'aot', 'aot')
                return 0, save_path
            else:
                return 0, data
...
def create_settings_export(save_path=None):
    try:
        ...
        with zipfile.ZipFile(data, mode='w') as z:
            try:
                z.write(SQL_DATABASE_AOT,
                        os.path.basename(SQL_DATABASE_AOT))
            except:
                logger.error(f"Could not find database file {SQL_DATABASE_AOT}")
...
def generate_output_usage_report():
    ...
    try:
        ...
        set_user_grp(report_path_file, 'aot', 'aot')
    except Exception:
