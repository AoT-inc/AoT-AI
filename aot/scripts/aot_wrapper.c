...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    if (strcmp(argv[1], "backup-create") == 0) {
      ...
      sprintf(cmd, "rm -rf /var/AoT-backups/AoT-backup-%s", argv[2]);
      system(cmd);
      ...
      sprintf(cmd, "service aot restart");
      system(cmd);
    } else if (strcmp(argv[1], "daemon_start") == 0) {
      sprintf(cmd, "service aot start");
      system(cmd);
    } else if (strcmp(argv[1], "daemon_stop") == 0) {
      sprintf(cmd, "service aot stop");
      system(cmd);
    } else if (strcmp(argv[1], "delete_upload_dir") == 0) {
      sprintf(cmd, "rm -rf /opt/AoT/upload");
      system(cmd);
    } else if (strcmp(argv[1], "frontend_reload") == 0) {
      sprintf(cmd, "service aotflask reload");
      system(cmd);
    } else if (strcmp(argv[1], "frontend_restart") == 0) {
      sprintf(cmd, "service aotflask restart");
      system(cmd);
      ...
      system(cmd);
    } else if (strcmp(argv[1], "influxdb_restore_aot_db") == 0 && (argc > 2)) {
      sprintf(cmd, "influxd restore -portable -db aot_db -newdb aot_db_bak %s", argv[2]);
      system(cmd);
    ...
    printf("AoT Wrapper: A wrapper to allow the aot web interface\n");
    printf("                to perform superuser actions.\n\n");
    printf("Usage: aot_wrapper [command]\n\n");
    printf("Commands:\n");
    printf("   backup-create:              Create AoT backup\n");
    printf("   backup-delete [DIRECTORY]:  Delete AoT backup [DIRECTORY]\n");
    printf("   backup-restore [DIRECTORY]: Restore AoT from backup [DIRECTORY]\n");
    printf("   daemon_restart:             Restart the aot daemon in normal mode\n");
    printf("   daemon_restart_debug:       Restart the aot daemon in debug mode\n");
    printf("   daemon_start:               Start the aot daemon\n");
    printf("   daemon_stop:                Stop the aot daemon\n");
    ...
    printf("   frontend_reload:            Reload the aot frontend\n");
    printf("   frontend_restart:           Restart the aot frontend\n");
    printf("   influxdb_start:             Start influxdb\n");
    printf("   influxdb_stop:              Stop influxdb\n");
    printf("   influxdb_restart:           Restart influxdb\n");
    printf("   influxdb_restore_aot_db  Restore influxdb database aot_db to aot_db_bak\n");
    printf("   initialize:                 Run the AoT initialization sequence\n");
    printf("   update_dependencies:        Install unmet dependencies and update installed dependencies\n");
    printf("   update_permissions:         Set the AoT file/folder permissions\n");
    printf("   restart:                    Restart the computer after a 10 second pause\n");
    printf("   shutdown:                   Shutdown the computer after a 10 second pause\n");
    printf("   upgrade-release-wipe:       Upgrade AoT to the latest major version on github and wipe database and virtualenv\n");
    printf("   upgrade-release-major:      Upgrade AoT to the latest major version on github\n");
    printf("   upgrade-master:             Upgrade AoT to the latest master branch on github\n");
  }
