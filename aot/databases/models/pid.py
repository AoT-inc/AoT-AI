...
class PID(CRUDMixin, db.Model):
    """
    Represents a PID (Proportional-Integral-Derivative) controller for automated regulation.

    PIDs read a measurement from a sensor, compare it to a setpoint, and drive
    output devices (raise/lower) to maintain the setpoint. Supports autotune,
    setpoint tracking, integrator limits, and preset configurations.
    ...
    timezone = db.Column(db.String(64), default=None)
...
class PIDSchema(ma.SQLAlchemyAutoSchema):
    """
    Marshmallow schema for serializing and deserializing PID instances.
