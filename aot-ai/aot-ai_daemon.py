#!/opt/AoT-AI/env/bin/python
# -*- coding: utf-8 -*-
#
#  aot-ai_daemon.py - Daemon for managing AoT-AI controllers, such as sensors,
...
from aot-ai.config import (DAEMON_LOG_FILE, DOCKER_CONTAINER, AOT-AI_DB_PATH,
                           AOT-AI_VERSION, STATS_CSV, STATS_INTERVAL,
                           UPGRADE_CHECK_INTERVAL)
from aot-ai.controllers.controller_conditional import ConditionalController
from aot-ai.controllers.controller_function import FunctionController
from aot-ai.controllers.controller_input import InputController
from aot-ai.controllers.controller_output import OutputController
from aot-ai.controllers.controller_pid import PIDController
from aot-ai.controllers.controller_trigger import TriggerController
from aot-ai.controllers.controller_widget import WidgetController
from aot-ai.databases.models import (PID, Camera, Conditional,
                                     CustomController, Input, Misc, Trigger)
from aot-ai.databases.utils import session_scope
from aot-ai.devices.camera import camera_record
from aot-ai.utils.actions import (get_condition_value,
...
from aot-ai.utils.github_release_info import AoT-AIRelease
from aot-ai.utils.stats import (add_update_csv, recreate_stat_file,
                                return_stat_file_dict, send_anonymous_stats)
from aot-ai.utils.tools import generate_output_usage_report, next_schedule
...
logger = logging.getLogger('aot-ai')
...
class DaemonController:
    ...
    def __init__(self):
        ...
        self.logger.info(f"AoT-AI daemon v{AOT-AI_VERSION} starting")
    ...
    def run(self):
        ...
        self.logger.info(
            f"AoT-AI daemon started in {self.startup_time:.3f} seconds")
        ...
        while self.daemon_run:
            ...
            try:
                ...
                if self.enable_upgrade_check and now > self.timer_upgrade:
                    ...
                    self.check_aot-ai_upgrade_exists(now)
        ...
        self.logger.info(f"AoT-AI daemon terminated in {timer:.3f} seconds\n\n")
    ...
    def check_aot-ai_upgrade_exists(self, now):
        """Check for any new AoT-AI releases on github."""
        try:
            aot-ai_releases = AoT-AIRelease()
            (upgrade_exists, _, _, _, errors) = aot-ai_releases.github_upgrade_exists()
            ...
            if upgrade_exists:
                ...
                if now > self.timer_upgrade_message:
                    ...
                    self.logger.info(
                        "A new version of AoT-AI is available. Upgrade "
                        "through the web interface under Config -> Upgrade. "
                        "This message will repeat every 10 days unless "
                        "AoT-AI is upgraded or upgrade checks are disabled.")
            else:
                upgrade_available = False

            with session_scope(AOT-AI_DB_PATH) as new_session:
                mod_misc = new_session.query(Misc).first()
                if mod_misc.aot-ai_upgrade_available != upgrade_available:
                    mod_misc.aot-ai_upgrade_available = upgrade_available
                    new_session.commit()
        except Exception:
            self.logger.exception("AoT-AI Upgrade Check ERROR")
...
class PyroServer(object):
    ...
    """
    def __init__(self, aot-ai):
        self.aot-ai = aot-ai

    def lcd_reset(self, lcd_id):
        """Resets an LCD."""
        return self.aot-ai.lcd_reset(lcd_id)

    def lcd_backlight(self, lcd_id, state):
        """Turns an LCD backlight on or off."""
        return self.aot-ai.lcd_backlight(lcd_id, state)

    def display_backlight_color(self, lcd_id, color):
        """Set the LCD backlight color."""
        return self.aot-ai.display_backlight_color(lcd_id, color)

    def lcd_flash(self, lcd_id, state):
        """Starts or stops an LCD from flashing (alarm)"""
        return self.aot-ai.lcd_flash(lcd_id, state)

    def get_condition_measurement(self, condition_id):
        return self.aot-ai.get_condition_measurement(condition_id)

    def get_condition_measurement_dict(self, condition_id):
        return self.aot-ai.get_condition_measurement_dict(condition_id)

    def module_function(self, controller_type, unique_id, button_id, args_dict, thread=True, return_from_function=False):
        """execute custom button function."""
        return self.aot-ai.module_function(
            controller_type, unique_id, button_id, args_dict, thread=thread, return_from_function=return_from_function)

    def controller_activate(self, cont_id):
        """Activates a controller."""
        try:
            return self.aot-ai.controller_activate(cont_id)
        except Exception as except_msg:
    ...
    def controller_deactivate(self, cont_id):
        ...
        try:
            return self.aot-ai.controller_deactivate(cont_id)
        except Exception as except_msg:
    ...
    def controller_restart(self, cont_id):
        ...
        try:
            return self.aot-ai.controller_restart(cont_id)
        except Exception as except_msg:
    ...
    def controller_is_active(self, cont_id):
        """Checks if a controller is active."""
        return self.aot-ai.controller_is_active(cont_id)

    def check_daemon(self):
        """Check if all active controllers respond."""
        return self.aot-ai.check_daemon()

    def function_status(self, function_id):
        """Get status of Function."""
        return self.aot-ai.function_status(function_id)

    def input_force_measurements(self, input_id):
        """Updates all input information."""
        return self.aot-ai.input_force_measurements(input_id)

    def pid_hold(self, pid_id):
        """Hold PID Controller operation."""
        return self.aot-ai.pid_hold(pid_id)

    def pid_mod(self, pid_id):
        """Set new PID Controller settings."""
        return self.aot-ai.pid_mod(pid_id)

    def pid_pause(self, pid_id):
        """Pause PID Controller operation."""
        return self.aot-ai.pid_pause(pid_id)

    def pid_resume(self, pid_id):
        """Resume PID controller operation."""
        return self.aot-ai.pid_resume(pid_id)

    def pid_get(self, pid_id, setting):
        """Get PID setting."""
        return self.aot-ai.pid_get(pid_id, setting)

    def pid_set(self, pid_id, setting, value):
        """Set PID setting."""
        return self.aot-ai.pid_set(pid_id, setting, value)

    def refresh_daemon_conditional_settings(self, unique_id):
        """Instruct the daemon to refresh a conditional's settings."""
        return self.aot-ai.refresh_daemon_conditional_settings(unique_id)

    def refresh_daemon_misc_settings(self):
        """Instruct the daemon to refresh the misc settings."""
        return self.aot-ai.refresh_daemon_misc_settings()

    def refresh_daemon_trigger_settings(self, unique_id):
        """Instruct the daemon to refresh a conditional's settings."""
        return self.aot-ai.refresh_daemon_trigger_settings(unique_id)

    def output_state(self, output_id, output_channel):
        """Return the output state (on or off)"""
        return self.aot-ai.output_state(output_id, output_channel)

    def output_states_all(self):
        """Return all output states."""
        return self.aot-ai.output_states_all()
    ...
    def output_sec_currently_on(self, output_id, output_channel=None):
        ...
        return self.aot-ai.controller['Output'].output_sec_currently_on(
            output_id, output_channel=output_channel)

    def output_setup(self, action, output_id):
        """Add, delete, or modify a output in the running output controller."""
        return self.aot-ai.output_setup(action, output_id)

    def trigger_action(self, action_id, value={}, debug=False):
        """Trigger action."""
        return self.aot-ai.trigger_action(
            action_id,
            value=value,
            debug=debug)
    ...
    def trigger_all_actions(self, function_id, message='', debug=False):
        """Trigger all actions."""
        return self.aot-ai.trigger_all_actions(function_id, message=message, debug=debug)

    def terminate_daemon(self):
        """Instruct the daemon to shut down."""
        return self.aot-ai.terminate_daemon()

    def widget_add_refresh(self, unique_id):
        """Add or refresh widget object."""
        return self.aot-ai.controller['Widget'].widget_add_refresh(unique_id)

    def widget_remove(self, unique_id):
        """Remove widget object."""
        return self.aot-ai.controller['Widget'].widget_remove(unique_id)

    def widget_execute(self, unique_id):
        """Execute widget object."""
        return self.aot-ai.controller['Widget'].widget_execute(unique_id)
...
class PyroDaemon(threading.Thread):
    ...
    controllers. It allows the client (aot-ai_client.py to be executed as non-root
    user) to communicate with the daemon (aot-ai_daemon.py running with root privileges).

    """
    def __init__(self, aot-ai):
        threading.Thread.__init__(self)

        self.logger = logging.getLogger('aot-ai.pyro_daemon')
        self.aot-ai = aot-ai
    ...
    def run(self):
        try:
            ...
            serve({
                PyroServer(self.aot-ai): 'aot-ai.pyro_server',
...
class PyroMonitor(threading.Thread):
    ...
    def __init__(self):
        ...
        self.logger = logging.getLogger('aot-ai.pyro_monitor')
        self.timer_sec = 1800
        ...
        try:
            ...
            while True:
                ...
                if now > log_timer:
                    ...
                    try:
                        proxy = Proxy("PYRO:aot-ai.pyro_server@127.0.0.1:9080")
                        proxy.check_daemon()
...
class AoT-AIDaemon:
    """
    Handle starting the components of the AoT-AI Daemon
    """
    def __init__(self, aot-ai):
        self.logger = logging.getLogger('aot-ai.daemon')
        self.aot-ai = aot-ai

    def start_daemon(self):
        """Start communication and daemon threads."""
        try:
            pd = PyroDaemon(self.aot-ai)
            pd.daemon = True
            pd.start()
            ...
            self.aot-ai.run()  # Start daemon thread that manages controllers
        except Exception:
            self.logger.exception("ERROR Starting AoT-AI Daemon")


def parse_args():
    parser = argparse.ArgumentParser(description='AoT-AI daemon.')
...
if __name__ == '__main__':
    ...
    aot-ai_daemon = AoT-AIDaemon(daemon_controller)
    aot-ai_daemon.start_daemon()
