...
def execute_at_modification(
    ...
    try:
        ...
        cmd_test = 'mkdir -p /opt/AoT-AI/.pylint.d && ' \
                   'export PYTHONPATH=$PYTHONPATH:/opt/AoT-AI && ' \
                   'export PYLINTHOME=/opt/AoT-AI/.pylint.d && ' \
...
INPUT_INFORMATION = {
    ...
    'message': 'All channels require a Measurement Unit to be selected and saved in order to store values to the '
               'database. Your code is executed from the same Python virtual environment that AoT-AI runs from. '
               'Therefore, you must install Python libraries to this environment if you want them to be available to '
               'your code. This virtualenv is located at /opt/AoT-AI/env and if you wanted to install a library, for '
               'example "my_library" using pip, you would execute "sudo /opt/AoT-AI/env/bin/pip install my_library".',
...
class InputModule(AbstractInput):
    ...
    def get_measurement(self):
        ...
        module_name = "aot-ai.input.python_code_exec_{}".format(os.path.basename(file_run).split('.')[0])
