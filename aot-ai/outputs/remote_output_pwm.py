# coding=utf-8
#
# remote_output_pwm.py - Output for controlling a remote AoT-AI PWM Output
#
import copy
import json
import time
...
from aot-ai.databases.models import DeviceMeasurements, OutputChannel
from aot-ai.outputs.base_output import AbstractOutput
from aot-ai.utils.database import db_retrieve_table_daemon
from aot-ai.utils.influx import add_measurements_influxdb, read_influxdb_single
...
OUTPUT_INFORMATION = {
    ...
    'output_name': "{} AoT-AI Output: {}".format(lazy_gettext('Remote'), lazy_gettext('PWM')),
    'measurements_dict': measurements_dict,
    'channels_dict': channels_dict,
    'output_library': 'requests',
    'output_types': ['pwm'],

    'message': 'This Output allows remote control of another AoT-AI PWM Output over a network using the API.',
    ...
    'custom_options_message': 'Enter the API key and IP/Host address of your remote AoT-AI and save to populate the Remote Output dropdown selection. You will need to refresh the page after saving for the Remote AoT-AI Output dropdown to populate. Configure which Remote AoT-AI Output you would like to control and save again. You must select an On/Off Output Channel for this to work. Selecting a PWM, Volume, or other channel will cause an error.',
    ...
    'custom_options': [
        {
            ...
            'default_value': '',
            'required': True,
            'name': "Remote AoT-AI Host",
            'phrase': lazy_gettext('The host or IP address of the remote AoT-AI')
        },
        {
            'id': 'api_key',
            'type': 'text',
            'default_value': '',
            'required': True,
            'name': "Remote AoT-AI API Key",
            'phrase': lazy_gettext('The API key of the remote AoT-AI')
        },
        {
    ...
    'custom_channel_options': [
        {
            ...
            'default_value': '',
            'name': 'Remote AoT-AI Output',
            'phrase': 'The Remote AoT-AI Output to control'
        },
        {
            'id': 'state_startup',
            'type': 'select',
