#!/bin/bash
#
#  upgrade_post.sh - Commands to execute after a AoT-AI upgrade
#
...
# Delete /env if AoT-AI symlink target isn't /opt/AoT-AI
if [ "$(readlink /var/aot-ai-root)" != "/opt/AoT-AI" ] ; then
...
DB_INFO=$( ${INSTALL_DIRECTORY}/env/bin/python ${INSTALL_DIRECTORY}/aot-ai/scripts/measurement_db.py -i )
...
TIMER_START_update_aot-ai_startup_script=$SECONDS
${INSTALL_CMD} update-aot-ai-startup-script
TIMER_TOTAL_update_aot-ai_startup_script=$((SECONDS - TIMER_START_update_aot-ai_startup_script))
...
printf "\nupdate-aot-ai-startup-script: %s s" "${TIMER_TOTAL_update_aot-ai_startup_script}"
