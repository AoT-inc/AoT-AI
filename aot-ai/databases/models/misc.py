...
from aot-ai.databases import CRUDMixin, set_uuid
from aot-ai.aot-ai_flask.extensions import db

logger = logging.getLogger("aot-ai.misc")
...
class Misc(CRUDMixin, db.Model):
    ...
    enable_upgrade_check = db.Column(db.Boolean, default=True)  # Periodically check for a AoT-AI upgrade
    aot-ai_upgrade_available = db.Column(db.Boolean, default=False)  # Stores if an upgrade is available
    ...
    measurement_db_user = db.Column(db.String(36), default='aot-ai')
    measurement_db_password = db.Column(db.String(36), default='mmdu77sj3nIoiajjs')
    measurement_db_dbname = db.Column(db.String(36), default='aot-ai_db')
