...
from aot-ai.config import (BACKUP_LOG_FILE, BACKUP_PATH, CAMERA_INFO,
                           DEPENDENCIES_GENERAL, DEPENDENCY_INIT_FILE,
                           DEPENDENCY_LOG_FILE, FINAL_RELEASES,
                           FORCE_UPGRADE_MASTER, FUNCTION_INFO,
                           INSTALL_DIRECTORY, METHOD_INFO, AOT-AI_VERSION,
...
from aot-ai.aot-ai_flask.forms import forms_dependencies, forms_misc
from aot-ai.aot-ai_flask.routes_static import inject_variables
...
from aot-ai.utils.stats import return_stat_file_dict
from aot-ai.utils.system_pi import (can_perform_backup, cmd_output,
                                    get_directory_size, internet)
from aot-ai.utils.widgets import parse_widget_information

logger = logging.getLogger('aot-ai.aot-ai_flask.admin')
...
def admin_backup():
    ...
    backup_dirs_tmp = []
    if not os.path.isdir('/var/AoT-AI-backups'):
    ...
    for each_dir in backup_dirs_tmp:
        if each_dir.startswith("AoT-AI-backup-"):
    ...
    if request.method == 'POST':
        if form_backup.backup.data:
            ...
            if free_after / 1000000 > 50:
                cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper backup-create" \
                      " | ts '[%Y-%m-%d %H:%M:%S]'" \
            ...
            try:
                ...
                download_dir = os.path.join(BACKUP_PATH, 'AoT-AI-backup-{}'.format(backup_date_version))
                save_file = "AoT-AI_Backup_{dv}_{host}_.zip".format(
                ...
                if not os.path.isdir(download_dir):
                    ...
                    with zipfile.ZipFile(data, 'w') as zipf:
                        ...
                        for file in file_paths:
                            # Remove first two directory names from zip file, so the AoT-AI root is the zip root
            ...
            cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper backup-delete {dir}" \
            ...
            if not os.path.isdir(form_backup.full_path.data):
                ...
                cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper backup-restore {backup}" \
                      " | ts '[%Y-%m-%d %H:%M:%S]'" \
                      " >> {log} 2>&1".format(pth=INSTALL_DIRECTORY,
...
def install_dependencies(dependencies):
    ...
    for each_dep in dependencies:
        if each_dep[2] == 'bash-commands':
            ...
            cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper install_dependency {dep}" \
    ...
    cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper update_permissions" \
          " | ts '[%Y-%m-%d %H:%M:%S]' >> {log}  2>&1".format(
            pth=INSTALL_DIRECTORY,
            log=DEPENDENCY_LOG_FILE)
...
def admin_upgrade():
    ...
    if upgrade:
        if upgrade == 2:
            flash(gettext("There was an error encountered during the upgrade"
                  ...
                  "error")
        return render_template('admin/upgrade.html',
                               current_release=AOT-AI_VERSION,
    ...
    aot-ai_releases_check = AoT-AIRelease()
    (upgrade_exists,
     releases,
     aot-ai_releases,
     current_latest_release,
     errors) = aot-ai_releases_check.github_upgrade_exists()
    ...
    if releases and current_latest_release and "." in current_latest_release:
        ...
        for index, each_release in enumerate(releases):
            if parse_version(each_release) >= parse_version(AOT-AI_VERSION):
        ...
        if (form_upgrade.upgrade.data and
            ...
            if not_enough_space_upgrade():
                ...
                try:
                    ...
                    pass
                cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper upgrade-master" \
                ...
                try:
                    ...
                    pass
                cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper upgrade-release-major {current_maj_version}" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' 2>&1 | tee -a {log} {tmp_log}".format(
                    current_maj_version=AOT-AI_VERSION.split('.')[0],
                    pth=INSTALL_DIRECTORY,
                    log=UPGRADE_LOG_FILE,
                    tmp_log=UPGRADE_TMP_LOG_FILE)
                subprocess.Popen(cmd, shell=True)
                ...
                upgrade = 1
                mod_misc = Misc.query.first()
                mod_misc.aot-ai_upgrade_available = False
                db.session.commit()
            ...
            if not not_enough_space_upgrade():
                try:
                    ...
                    pass
                cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper upgrade-release-wipe {ver}" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' 2>&1 | tee -a {log} {tmp_log}".format(
                    pth=INSTALL_DIRECTORY,
