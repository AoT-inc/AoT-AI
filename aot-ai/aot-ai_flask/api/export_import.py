...
from aot-ai.aot-ai_flask.api import api, default_responses
from aot-ai.aot-ai_flask.utils.utils_export import (export_influxdb,
                                                    export_settings)
...
class BackupInfluxdb(Resource):
    """Download a AoT-AI configuration export."""

    @accept('application/vnd.aot-ai.v1+json')
    @flask_login.login_required
    def get(self):
        """
        Return an archive of the AoT-AI confgiuration export.
        """
        if not user_has_permission('view_settings'):
            abort(403)
