...
from aot-ai.config import (DOCKER_CONTAINER, INSTALL_DIRECTORY, LOG_PATH,
                           PATH_CAMERAS, PATH_NOTE_ATTACHMENTS)
from aot-ai.databases.models import (PID, Camera, Conversion, CustomController,
                                     DeviceMeasurements, Input, Misc, Notes,
                                     NoteTags, Output, OutputChannel)
from aot-ai.aot-ai_client import DaemonControl
from aot-ai.aot-ai_flask.routes_authentication import clear_cookie_auth
from aot-ai.aot-ai_flask.utils import utils_general
from aot-ai.aot-ai_flask.utils.utils_general import get_ip_address
from aot-ai.aot-ai_flask.utils.utils_output import get_all_output_states
from aot-ai.utils.database import db_retrieve_table
from aot-ai.utils.influx import (influx_to_list, influxdb_get_count_points,
                                 influxdb_get_first_point, query_string)
from aot-ai.utils.system_pi import (assure_path_exists, is_int,
...
def video_feed(unique_id):
    ...
    camera_stream = import_module('aot-ai.aot-ai_flask.camera.camera_' + camera_options.library).Camera
    ...
    try:
        ...
        if DOCKER_CONTAINER:
            if action == 'daemon_restart':
                ...
                subprocess.Popen('docker restart aot-ai_flask 2>&1', shell=True)
        else:
            if action == 'frontend_reload':
                ...
                cmd = f'{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper {action} 2>&1'
                subprocess.Popen(cmd, shell=True)
...
#
#     if success:
#         from aot-ai.utils.image import generate_thermal_image_from_pixels
