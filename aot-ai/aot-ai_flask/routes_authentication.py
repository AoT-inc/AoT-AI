...
from aot-ai.config import (INSTALL_DIRECTORY, LANGUAGES, LOGIN_ATTEMPTS,
                           LOGIN_BAN_SECONDS, LOGIN_LOG_FILE)
from aot-ai.config_translations import TRANSLATIONS
from aot-ai.databases.models import AlembicVersion, Misc, Role, User
from aot-ai.aot-ai_flask.extensions import db
from aot-ai.aot-ai_flask.forms import forms_authentication
from aot-ai.aot-ai_flask.utils import utils_general
from aot-ai.utils.utils import test_password, test_username
...
def create_admin():
    ...
    # Find user-selected language in AoT-AI/.language
    try:
    ...
    if banned_from_login():
        ...
        if request.method == 'POST':
            if form_language.language.data:
                ...
                username = form_login.aot-ai_username.data.lower()
                ...
                if not user:
                    ...
                    matched_hash = User().check_password(
                        form_login.aot-ai_password.data, user.password_hash)
...
def newremote():
    ...
    if user:
        if User().check_password(
            ...
            try:
                with open('/opt/AoT-AI/aot-ai/aot-ai_flask/ssl_certs/cert.pem', 'r') as cert:
...
def check_database_version_issue():
    if len(AlembicVersion.query.all()) > 1:
        flash("A check of your database indicates there is an issue with your"
              ...
              " your aot-ai.db from /opt/AoT-AI/databases/aot-ai.db to a "
