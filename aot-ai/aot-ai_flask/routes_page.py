...
from aot-ai.config import (ALEMBIC_VERSION, BACKUP_LOG_FILE, CAMERA_INFO,
                           DAEMON_LOG_FILE,
                           DEPENDENCY_LOG_FILE, DOCKER_CONTAINER,
                           FRONTEND_PID_FILE, HTTP_ACCESS_LOG_FILE,
                           HTTP_ERROR_LOG_FILE, IMPORT_LOG_FILE,
                           KEEPUP_LOG_FILE, LOGIN_LOG_FILE, AOT-AI_DB_PATH,
                           AOT-AI_VERSION, RESTORE_LOG_FILE, THEMES_DARK,
                           UPGRADE_LOG_FILE)
from aot-ai.config_devices_units import MEASUREMENTS
from aot-ai.databases.models import (PID, AlembicVersion, Camera, Conversion,
...
from aot-ai.aot-ai_client import DaemonControl, daemon_active
from aot-ai.aot-ai_flask.extensions import db
from aot-ai.aot-ai_flask.forms import forms_camera, forms_misc, forms_notes
from aot-ai.aot-ai_flask.routes_static import inject_variables
from aot-ai.aot-ai_flask.utils import (utils_camera, utils_dashboard,
                                       utils_export, utils_general, utils_misc,
                                       utils_notes)
from aot-ai.aot-ai_flask.utils.utils_general import return_dependencies
from aot-ai.utils.functions import parse_function_information
from aot-ai.utils.inputs import (list_analog_to_digital_converters,
                                 parse_input_information)
from aot-ai.utils.outputs import output_types, parse_output_information
from aot-ai.utils.system_pi import (
...
from aot-ai.utils.tools import (calc_energy_usage, return_energy_usage,
                                return_output_usage)

logger = logging.getLogger('aot-ai.aot-ai_flask.routes_page')
...
def page_camera():
    ...
    for each_cam in camera:
        if each_cam.library == 'opencv':
            try:
                from aot-ai.devices.camera import count_cameras_opencv
            ...
            if mod_camera.stream_started:
                try:
                    camera_stream = import_module(
                        'aot-ai.aot-ai_flask.camera.camera_{lib}'.format(
        ...
        if form_note_show.notes_show.data:
            ...
            if data:
                ...
                return send_file(
                    ...
                    as_attachment=True,
                    download_name=
                    'AoT-AI_Notes_{mv}_{host}_{dt}.zip'.format(
                        mv=AOT-AI_VERSION,
...
def page_logview():
    ...
    if form_log_view.log.data == 'log_nginx':
        if DOCKER_CONTAINER:
            command = f'docker logs -n {lines} aot-ai_nginx'
            if search:
        ...
        if DOCKER_CONTAINER:
            command = f'docker logs -n {lines} aot-ai_flask'
            if search:
                command += f' 2>&1 | grep "{search}"'
        else:
            command = f'journalctl -u aot-aiflask -n {lines} --no-pager'
            if search:
                command += f' | grep "{search}"'
