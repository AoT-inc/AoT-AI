...
from aot-ai.config import (ALEMBIC_VERSION, DATABASE_NAME, DOCKER_CONTAINER, IMPORT_LOG_FILE,
                              INSTALL_DIRECTORY, AOT-AI_VERSION,
                              ...
                              SQL_DATABASE_AOT-AI, DATABASE_PATH)
...
from aot-ai.scripts.measurement_db import get_influxdb_info
from aot-ai.utils.system_pi import assure_path_exists, cmd_output
from aot-ai.utils.tools import (create_measurements_export,
                                   create_settings_export)
from aot-ai.utils.utils import append_to_log
...
def export_settings():
    """
    AoT-AI 설정 데이터베이스(aot-ai.db)를 ZIP 파일로 저장하여 사용자에게 제공합니다.
    """
    action = '{action} {controller}'.format(
        action=TRANSLATIONS['export']['title'],
    ...
    try:
        ...
        if not status:
            return send_file(
                ...
                as_attachment=True,
                download_name=
                    'AoT-AI_{mver}_setup_{aver}_{host}_{dt}.zip'.format(
                        mver=AOT-AI_VERSION, aver=ALEMBIC_VERSION,
...
def export_influxdb():
    """
    AoT-AI InfluxDB 데이터베이스를 Enterprise 호환 형식으로 백업하여 ZIP 파일로 압축한 후 사용자에게 제공합니다.
    """
    action = '{action} {controller}'.format(
        action=TRANSLATIONS['export']['title'],
        controller=TRANSLATIONS['measurement']['title'])
        ...
        if influxdb_info['influxdb_host'] and influxdb_info['influxdb_version']:
            ...
            if not status:
                return send_file(
                    ...
                    as_attachment=True,
                    download_name=
                    'AoT-AI_{mv}_Influxdb_{iv}_{host}_{dt}.zip'.format(
                        mv=AOT-AI_VERSION, iv=influxdb_info['influxdb_version'],
                        host=socket.gethostname().replace(' ', ''),
                        dt=datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
                )
            else:
...
def thread_import_settings(tmp_folder):
    ...
    try:
        # 초기화
        cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper initialize | ts '[%Y-%m-%d %H:%M:%S]' >> {IMPORT_LOG_FILE} 2>&1"
        _, _, _ = cmd_output(cmd, user="root")

        # 데이터베이스 업그레이드
        append_to_log(IMPORT_LOG_FILE, f"\n[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 데이터베이스 업그레이드\n")
        cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper upgrade_database | ts '[%Y-%m-%d %H:%M:%S]' >> {IMPORT_LOG_FILE} 2>&1"
        _, _, _ = cmd_output(cmd, user="root")

        # 패키지 설치/업데이트 (시간이 걸릴 수 있음)
        append_to_log(IMPORT_LOG_FILE, f"\n[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 패키지 설치 중 (잠시만 기다려 주세요)...\n")
        cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper update_dependencies | ts '[%Y-%m-%d %H:%M:%S]' >> {IMPORT_LOG_FILE} 2>&1"
        _, _, _ = cmd_output(cmd, user="root")
        ...
        if DOCKER_CONTAINER:
            subprocess.Popen('docker start aot-ai_daemon 2>&1', shell=True)
        else:
            cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper daemon_restart | ts '[%Y-%m-%d %H:%M:%S]' >> {IMPORT_LOG_FILE} 2>&1"
        ...
        # AoT-AI Flask(프론트엔드) 재로딩
        append_to_log(IMPORT_LOG_FILE, f"\n[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 프론트엔드 재로딩")
        if DOCKER_CONTAINER:
            subprocess.Popen('docker start aot-ai_flask 2>&1', shell=True)
        else:
            cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper frontend_reload | ts '[%Y-%m-%d %H:%M:%S]' >> {IMPORT_LOG_FILE} 2>&1"
            _, _, _ = cmd_output(cmd, user="root")
    except:
...
def import_settings(form):
    """
    export_settings()로 내보낸 AoT-AI 설정 데이터베이스가 포함된 ZIP 파일을 받아,
    현재 AoT-AI 설정 데이터베이스를 백업한 후 ZIP 파일의 것으로 대체합니다.
    """
    action = '{action} {controller}'.format(
    ...
    try:
        ...
        tmp_folder = os.path.join(upload_folder, 'aot-ai_db_tmp')
        full_path = None
        ...
        if not form.settings_import_file.data:
            ...
            try:
                ...
                elif parse(name_split[1]) > parse(AOT-AI_VERSION):
                    error.append(gettext("잘못된 AoT-AI 버전: %(version)s > %(current)s. %(msg)s", version=name_split[1], current=AOT-AI_VERSION, msg=gettext("현재 버전 이하의 데이터베이스만 가져올 수 있습니다.")))
            except Exception as err:
        ...
        if not error:
            ...
            try:
                if DOCKER_CONTAINER:
                    subprocess.Popen('docker stop aot-ai_daemon 2>&1', shell=True)
                else:
                    # AoT-AI 데몬(백엔드) 중지
                    cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper daemon_stop"
                    _, _, _ = cmd_output(cmd, user="root")

                # 현재 데이터베이스 백업 및 가져온 aot-ai.db로 교체
                imported_database = os.path.join(tmp_folder, DATABASE_NAME)
                backup_name = f"{SQL_DATABASE_AOT-AI}.backup_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
                full_path_backup = os.path.join(DATABASE_PATH, backup_name)

                append_to_log(IMPORT_LOG_FILE,
                              f"\n\n[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {SQL_DATABASE_AOT-AI}을(를) {full_path_backup}로 이름 변경 중")
                os.rename(SQL_DATABASE_AOT-AI, full_path_backup)  # 현재 데이터베이스 백업
                append_to_log(IMPORT_LOG_FILE,
                              f"\n\n[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {imported_database}을(를) {SQL_DATABASE_AOT-AI}로 이동 중")
                shutil.move(imported_database, SQL_DATABASE_AOT-AI)  # 가져온 데이터베이스로 교체
