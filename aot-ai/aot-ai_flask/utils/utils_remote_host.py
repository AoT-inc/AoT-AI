...
from aot-ai.config import STORED_SSL_CERTIFICATE_PATH
from aot-ai.databases.models import DisplayOrder
from aot-ai.databases.models import Remote
from aot-ai.aot-ai_flask.extensions import db
from aot-ai.aot-ai_flask.utils import utils_general
from aot-ai.aot-ai_flask.utils.utils_general import add_display_order
...
from aot-ai.utils.system_pi import csv_to_list_of_str
from aot-ai.utils.system_pi import list_to_csv

logger = logging.getLogger(__name__)

#
# Remote host commands executed on AoT-AI with Remote Admin Dashboard
#

def remote_log_in(address, user, password_hash):
    """
    Log in to AoT-AI and return the cookie header for subsequent requests

    :param address: host name or IP address of remote AoT-AI
    :param user: User name of an admin on the remote AoT-AI
...
def remote_host_page(address, headers, page):
    """
    Make request, receive response, and authenticate a remote AoT-AI
...
def remote_host_add(form_setup, display_order):
    """
    Add a remote AoT-AI to the Remote Admin Dashboard

    Authenticate a remote AoT-AI install that will be used on this system's
    ...
    if form_setup.validate():
        try:
            # Send user and password to remote AoT-AI to authenticate
            credentials = {
...
def remote_host_del(form_setup):
    """Delete a remote AoT-AI from the Remote Admin Dashboard."""
    if not utils_general.user_has_permission('edit_settings'):
        return redirect(url_for('routes_general.home'))
