...
from aot-ai.config import DAEMON_LOG_FILE
from aot-ai.config import DEPENDENCY_INIT_FILE
from aot-ai.config import DEPENDENCY_LOG_FILE
from aot-ai.config import INSTALL_DIRECTORY
from aot-ai.config import PATH_ACTIONS_CUSTOM
from aot-ai.config import PATH_FUNCTIONS_CUSTOM
from aot-ai.config import PATH_INPUTS_CUSTOM
from aot-ai.config import PATH_OUTPUTS_CUSTOM
from aot-ai.config import PATH_TEMPLATE_USER
from aot-ai.config import PATH_WIDGETS_CUSTOM
from aot-ai.config import UPGRADE_INIT_FILE
from aot-ai.config_devices_units import MEASUREMENTS
from aot-ai.config_devices_units import UNITS
from aot-ai.config_translations import TRANSLATIONS
from aot-ai.databases import set_api_key
...
from aot-ai.databases.models import Dashboard
from aot-ai.databases.models import DeviceMeasurements
from aot-ai.databases.models import DisplayOrder
from aot-ai.databases.models import Input
from aot-ai.databases.models import Measurement
from aot-ai.databases.models import Misc
from aot-ai.databases.models import NoteTags
from aot-ai.databases.models import Notes
...
from aot-ai.databases.models import Role
from aot-ai.databases.models import SMTP
from aot-ai.databases.models import Unit
from aot-ai.databases.models import User
from aot-ai.databases.models import Widget
from aot-ai.aot-ai_client import DaemonControl
from aot-ai.aot-ai_flask.extensions import db
from aot-ai.aot-ai_flask.utils import utils_general
from aot-ai.aot-ai_flask.utils.utils_general import choices_measurements
from aot-ai.aot-ai_flask.utils.utils_general import choices_units
from aot-ai.aot-ai_flask.utils.utils_general import controller_activate_deactivate
from aot-ai.aot-ai_flask.utils.utils_general import delete_entry_with_id
from aot-ai.aot-ai_flask.utils.utils_general import flash_form_errors
from aot-ai.aot-ai_flask.utils.utils_general import flash_success_errors
from aot-ai.utils.actions import parse_action_information
from aot-ai.utils.database import db_retrieve_table
from aot-ai.utils.functions import parse_function_information
from aot-ai.utils.inputs import parse_input_information
from aot-ai.utils.layouts import update_layout
from aot-ai.utils.modules import load_module_from_file
from aot-ai.utils.outputs import parse_output_information
from aot-ai.utils.send_data import send_email
from aot-ai.utils.system_pi import all_conversions
from aot-ai.utils.system_pi import assure_path_exists
from aot-ai.utils.system_pi import base64_encode_bytes
from aot-ai.utils.system_pi import cmd_output
from aot-ai.utils.system_pi import set_user_grp
from aot-ai.utils.utils import test_password
from aot-ai.utils.utils import test_username
from aot-ai.utils.widget_generate_html import generate_widget_html
from aot-ai.utils.widgets import parse_widget_information
        ...
        if not messages["error"]:
            try:
                ...
                if reload_frontend:
                    ...
                    cmd = f"sleep 10 && {INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper frontend_reload 2>&1"
...
def settings_function_import(form):
    """
    Receive a function module file, check it for errors, add it to AoT-AI controller list
    """
    action = '{action} {controller}'.format(
    ...
    try:
        ...
        tmp_directory = os.path.join(install_dir, 'aot-ai/functions/tmp_functions')
        ...
        if not error:
            ...
            cmd = '{path}/aot-ai/scripts/aot-ai_wrapper frontend_reload 2>&1'.format(
                path=install_dir)
            subprocess.Popen(cmd, shell=True)
...
def settings_action_import(form):
    """
    Receive an action module file, check it for errors, add it to AoT-AI controller list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'aot-ai/actions/tmp_actions')
        assure_path_exists(tmp_directory)
...
def settings_input_import(form):
    """
    Receive an input module file, check it for errors, add it to AoT-AI input list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        # correct_format = 'AoT-AI_AOT-AIVERSION_Settings_DBVERSION_HOST_DATETIME.zip'
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'aot-ai/inputs/tmp_inputs')
        assure_path_exists(tmp_directory)
...
def settings_output_import(form):
    """
    Receive an output module file, check it for errors, add it to AoT-AI output list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        # correct_format = 'AoT-AI_AOT-AIVERSION_Settings_DBVERSION_HOST_DATETIME.zip'
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'aot-ai/outputs/tmp_outputs')
        assure_path_exists(tmp_directory)
...
def settings_widget_import(form):
    """
    Receive an widget module file, check it for errors, add it to AoT-AI widget list
    """
    action = '{action} {controller}'.format(
        action=gettext("Import"),
    ...
    try:
        # correct_format = 'AoT-AI_AOT-AIVERSION_Settings_DBVERSION_HOST_DATETIME.zip'
        install_dir = os.path.abspath(INSTALL_DIRECTORY)
        tmp_directory = os.path.join(install_dir, 'aot-ai/widgets/tmp_widgets')
        assure_path_exists(tmp_directory)
...
def settings_pi_mod(form):
    ...
    if form.enable_i2c.data:
        ...
        if form.pigpiod_sample_rate.data not in ['low', 'high', 'disabled', 'uninstalled']:
            ...
            # Stop the AoT-AI daemon
            cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper daemon_stop" \
            ...
            if (form.pigpiod_sample_rate.data != 'uninstalled' and
                ...
                cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper install_pigpiod" \
                      f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
                install_pigpiod = subprocess.Popen(cmd, shell=True)
                install_pigpiod.wait()

            # Disable pigpiod
            cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper disable_pigpiod" \
                  f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
            ...
            if form.pigpiod_sample_rate.data == 'low':
                # Install pigpiod (sample rate of 1 ms)
                cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper enable_pigpiod_low" \
                      f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
                ...
                cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper enable_pigpiod_high" \
                      f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
                ...
                cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper enable_pigpiod_disabled" \
                      f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
                disable_pigpiod = subprocess.Popen(cmd, shell=True)
                disable_pigpiod.wait()
            elif form.pigpiod_sample_rate.data == 'uninstalled':
                # Uninstall pigpiod (user selected disable)
                cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper uninstall_pigpiod" \
                      f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
                uninstall_pigpiod = subprocess.Popen(cmd, shell=True)
                uninstall_pigpiod.wait()

            # Start the AoT-AI daemon
            cmd = f"{INSTALL_DIRECTORY}/aot-ai/scripts/aot-ai_wrapper daemon_start" \
                  f" | ts '[%Y-%m-%d %H:%M:%S]' 2>&1"
        ...
        if form_mod_alert.validate():
            ...
            if form_mod_alert.send_test.data:
                send_email(
                    ...
                    "This is a test email from AoT-AI")
...
def settings_diagnostic_delete_settings_database():
    ...
    if not error:
        try:
            os.remove('/opt/AoT-AI/databases/aot-ai.db')
            cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper frontend_reload" \
                  " | ts '[%Y-%m-%d %H:%M:%S]'" \
...
def settings_diagnostic_recreate_influxdb_db_1():
    ...
    if not error:
        try:
            command = f'/bin/bash {INSTALL_DIRECTORY}/aot-ai/scripts/upgrade_commands.sh recreate-influxdb-1-db'
...
def settings_diagnostic_recreate_influxdb_db_2():
    ...
    if not error:
        try:
            command = f'/bin/bash {INSTALL_DIRECTORY}/aot-ai/scripts/upgrade_commands.sh recreate-influxdb-2-db'
            p = subprocess.Popen(command, shell=True)
            p.communicate()
        except Exception:
            logger.exception()
    ...
    if not error:
        try:
            def install_dependencies():
                cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper update_dependencies" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' >> {log} 2>&1".format(
                    pth=INSTALL_DIRECTORY,
                    log=DEPENDENCY_LOG_FILE)
                _, _, _ = cmd_output(cmd, user="root")

                cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper frontend_restart" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' >> {log} 2>&1".format(
                    pth=INSTALL_DIRECTORY,
                    log=DEPENDENCY_LOG_FILE)
                _, _, _ = cmd_output(cmd, user="root")

                cmd = "{pth}/aot-ai/scripts/aot-ai_wrapper daemon_restart" \
                      " | ts '[%Y-%m-%d %H:%M:%S]' >> {log} 2>&1".format(
                    pth=INSTALL_DIRECTORY,
                    log=DEPENDENCY_LOG_FILE)
                _, _, _ = cmd_output(cmd, user="root")
...
def settings_regenerate_widget_html():
    try:
        ...
        cmd = f'/bin/bash {INSTALL_DIRECTORY}/aot-ai/scripts/upgrade_commands.sh generate-widget-html'
...
def settings_diagnostic_upgrade_master():
    ...
    if not error:
        try:
            path_config = os.path.join(INSTALL_DIRECTORY, "aot-ai/config.py")
            ...
            set_user_grp(path_config, 'aot-ai', 'aot-ai')
            flash_success_errors(
                error, action, url_for('routes_settings.settings_diagnostic'))
            return
        except Exception as except_msg:
            ...
            command = '/bin/bash {path}/aot-ai/scripts/upgrade_commands.sh web-server-restart'.format(
