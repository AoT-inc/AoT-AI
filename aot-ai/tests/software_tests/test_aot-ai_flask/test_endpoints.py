...
from aot-ai.config import FUNCTIONS
from aot-ai.databases.models import (PID, Actions, Conditional,
                                     CustomController, Dashboard, Function,
                                     Input, Output, Trigger, User, Widget)
from aot-ai.aot-ai_flask.utils.utils_general import (choices_custom_functions,
                                                     generate_form_input_list,
                                                     generate_form_output_list,
                                                     generate_form_widget_list)
from aot-ai.tests.software_tests.conftest import login_user
from aot-ai.tests.software_tests.factories import UserFactory
...
def redirects_to_login_page(testapp, endpoint):
    ...
    assert "AoT-AI Login" in response
...
@mock.patch('aot-ai.aot-ai_flask.routes_authentication.login_log')
def test_api_logged_in_as_admin(_, testapp):
    ...
    for index, route in enumerate(api_endpoints):
        ...
        response = testapp.get(
            ...
            headers={'Accept': 'application/vnd.aot-ai.v1+json'})
...
def test_api_docs_when_not_logged_in(testapp):
    ...
    assert 'AoT-AI API' in response, "Unexpected HTTP Response: \n{body}".format(body=response.body)
...
def create_user(aot-ai_db, role_id, name, password):
    """Create fake admin user."""
    new_user = UserFactory()
    new_user.name = name
    new_user.set_password(password)
    new_user.role_id = role_id
    aot-ai_db.add(new_user)
    aot-ai_db.commit()
    return new_user
