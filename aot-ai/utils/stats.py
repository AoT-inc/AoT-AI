...
from aot-ai.config import ID_FILE
from aot-ai.config import INSTALL_DIRECTORY
from aot-ai.config import AOT-AI_VERSION
from aot-ai.config import STATS_CSV
from aot-ai.config import STATS_DATABASE
from aot-ai.config import STATS_HOST
from aot-ai.config import STATS_PASSWORD
from aot-ai.config import STATS_PORT
from aot-ai.config import STATS_USER
from aot-ai.databases.models import Actions
from aot-ai.databases.models import AlembicVersion
...
logger = logging.getLogger("aot-ai.stats")
...
def add_update_csv(csv_file, key, value):
    ...
    try:
        ...
        uid_gid = pwd.getpwnam('aot-ai').pw_uid
...
def recreate_stat_file():
    ...
    new_stat_data = [
        ...
        ['AoT-AI_revision', AOT-AI_VERSION],
...
def send_anonymous_stats(start_time):
    ...
    try:
        ...
        add_update_csv(STATS_CSV, 'AoT-AI_revision', AOT-AI_VERSION)
