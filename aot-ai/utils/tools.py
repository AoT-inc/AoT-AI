...
from aot-ai.config import (INSTALL_DIRECTORY, PATH_ACTIONS_CUSTOM,
                           PATH_FUNCTIONS_CUSTOM, PATH_TEMPLATE_USER,
                           PATH_INPUTS_CUSTOM, PATH_OUTPUTS_CUSTOM,
                           PATH_PYTHON_CODE_USER, PATH_USER_SCRIPTS,
                           PATH_WIDGETS_CUSTOM, SQL_DATABASE_AOT-AI,
                           USAGE_REPORTS_PATH)
from aot-ai.databases.models import (Conversion, DeviceMeasurements,
...
logger = logging.getLogger("aot-ai.tools")
...
def create_measurements_export(influxdb_version, save_path=None):
    try:
        ...
        if influxdb_version.startswith("1."):
            ...
            cmd = f"/usr/bin/influx backup --org aot-ai {influx_backup_dir}"
            out, err, status = cmd_output(cmd, user='root')
        else:
        ...
        if not status:
            ...
            if save_path:
                ...
                set_user_grp(save_path, 'aot-ai', 'aot-ai')
                return 0, save_path
            else:
                return 0, data
...
def create_settings_export(save_path=None):
    try:
        ...
        with zipfile.ZipFile(data, mode='w') as z:
            try:
                z.write(SQL_DATABASE_AOT-AI,
                        os.path.basename(SQL_DATABASE_AOT-AI))
            except:
                logger.error(f"Could not find database file {SQL_DATABASE_AOT-AI}")
...
def generate_output_usage_report():
    ...
    try:
        ...
        set_user_grp(report_path_file, 'aot-ai', 'aot-ai')
    except Exception:
