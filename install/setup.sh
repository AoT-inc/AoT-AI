#!/bin/bash
#
#  setup.sh - AoT-AI install script
#
#  Usage: sudo /bin/bash /opt/AoT-AI/install/setup.sh
#

INSTALL_DIRECTORY=$( cd "$( dirname "${BASH_SOURCE[0]}" )/.." && pwd -P )
INSTALL_CMD="/bin/bash ${INSTALL_DIRECTORY}/aot-ai/scripts/upgrade_commands.sh"
...
if [[ "$INSTALL_DIRECTORY" == "/opt/AoT-AI" ]]; then
  printf "## Currently installing with /opt/AoT-AI/install/setup.sh\n"
elif [[ "$INSTALL_DIRECTORY" != "/opt/AoT-AI" && ! -d /opt/AoT-AI ]]; then
  printf "## Current install directory (${INSTALL_DIRECTORY}) is not /opt/AoT-AI and /opt/AoT-AI doesn't exist. Copying and installing...\n"
  sudo cp -Rp "${INSTALL_DIRECTORY}" /opt/AoT-AI
  sudo /opt/AoT-AI/install/setup.sh
  exit 1
elif [[ "$INSTALL_DIRECTORY" != "/opt/AoT-AI" && -d /opt/AoT-AI ]]; then
  printf "## Error: Install aborted. /opt/AoT-AI exists and you're running setup from ${INSTALL_DIRECTORY}. Install not possible if a previous install is detected. Move or delete /opt/AoT-AI and rerun this script or run /opt/AoT-AI/install/setup.sh\n"
  exit 1
fi

# Fix for below issue(s)
# https://github.com/pypa/setuptools/issues/3278
# https://github.com/kizniche/AoT-AI/issues/1149
...
if hash python3 2>/dev/null; then
  if ! python3 "${INSTALL_DIRECTORY}"/aot-ai/scripts/upgrade_check.py --min_python_version "3.8"; then
...
if [ $exitstatus != 0 ]; then
    printf "\nError: dialog not installed. Install it then try the AoT-AI setup again.\n"
    exit 1
fi

START_A=$(date)
printf "### AoT-AI installation initiated %s\n" "${START_A}" 2>&1 | tee -a "${LOG_LOCATION}"

clear
LICENSE=$(dialog --title "AoT-AI Installer: License Agreement" \
                   --backtitle "AoT-AI" \
                   --yesno "AoT-AI is free software: you can redistribute it and/or modify it under the terms of the GNU General Public License as published by the Free Software Foundation, either version 3 of the License, or (at your option) any later version.\n\nAoT-AI is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU General Public License for more details.\n\nYou should have received a copy of the GNU General Public License along with AoT-AI. If not, see gnu.org/licenses\n\nDo you agree to the license terms?" \
                   20 68 \
                   3>&1 1>&2 2>&3)
exitstatus=$?
if [ $exitstatus != 0 ]; then
    printf "AoT-AI install cancelled by user\n" 2>&1 | tee -a "${LOG_LOCATION}"
    exit 1
fi

clear
LANGUAGE=$(dialog --title "AoT-AI Installer" \
                  --backtitle "AoT-AI" \
...
clear
INSTALL=$(dialog --title "AoT-AI Installer: Install" \
                   --backtitle "AoT-AI" \
                   --yesno "AoT-AI will be installed in the home directory of the current user. Several software packages will be installed via apt, including the nginx web server that the AoT-AI web user interface will be hosted on. Proceed with the installation?" \
                   20 68 \
                   3>&1 1>&2 2>&3)
exitstatus=$?
if [ $exitstatus != 0 ]; then
...
if [[ ${MACHINE_TYPE} == 'armhf' ]]; then
    INFLUX_A=$(dialog --title "AoT-AI Installer: Measurement Database" \
                        --backtitle "AoT-AI" \
                        --menu "Install Influxdb?\n\nIf you do not install InfluxDB now, you will need to set the InfluxDB server/credential settings in the Configuration after AoT-AI is installed." 20 68 4 \
    ...
    INFLUX_B=$(dialog --title "AoT-AI Installer: Measurement Database" \
                        --backtitle "AoT-AI" \
                        --menu "Install Influxdb?\n\nIf you do not install InfluxDB now, you will need to configure the InfluxDB server settings and credentials after AoT-AI is installed. If using a 32-bit CPU, you can only use 1.x (do not use 2.x, as it only works with 64-bit CPUs)." 20 68 4 \
...
if [[ ${INFLUX_A} == '1)' || ${INFLUX_B} == '2)' ]]; then
    clear
    INSTALL=$(dialog --title "AoT-AI Installer: Measurement Database" \
                       --backtitle "AoT-AI" \
                       --yesno "You have chosen not to install Influxdb. This is typically done if you want to use an existing influxdb server. Make sure to change the influxdb client options in the AoT-AI Configuration after installing to ensure measurements can be properly saved/queried. If you would like to install influxdb, select cancel and start the AoT-AI Installer over again." \
                       20 68 \
                       3>&1 1>&2 2>&3)
    exitstatus=$?
    if [ $exitstatus != 0 ]; then
...
abort()
{
    printf "
**********************************
** ERROR During AoT-AI Install! **
**********************************

An error occurred that may have prevented AoT-AI from
...
at https://github.com/kizniche/AoT-AI/issues with the
...
SECONDS=0
START_B=$(date)
printf "#### AoT-AI installation began %s\n" "${START_B}" 2>&1 | tee -a "${LOG_LOCATION}"
...
${INSTALL_CMD} update-aot-ai-startup-script 2>&1 | tee -a "${LOG_LOCATION}"
...
END=$(date)
printf "#### AoT-AI Installer finished %s\n" "${END}" 2>&1 | tee -a "${LOG_LOCATION}"
