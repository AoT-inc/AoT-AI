...
case "${1:-''}" in
    ...
    ;;
    "test")
        docker exec -ti aot-ai_flask "${INSTALL_PATH}"/env/bin/python -m pip install --upgrade -r /home/aot-ai/aot-ai/install/requirements-testing.txt
        docker exec -ti aot-ai_flask "${INSTALL_PATH}"/env/bin/python -m pytest /home/aot-ai/aot-ai/tests/software_tests
    ;;
    "clean-all")
