...
- Check the Logs: From the `[Gear Icon] -> AoT Logs` page or /var/log/aot/, check the daemon log for any errors. If the issue began after an upgrade, also check the upgrade log for indications of an issue.
...
- Check the Upgrade Log for any issues that may have occurred. The log is located at `/var/log/aot/aotupgrade.log` but may also be accessed from the web UI (if you're able to): select `[Gear Icon] -> AoT Logs -> Upgrade Log`.
