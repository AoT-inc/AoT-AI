...
ACTION_INFORMATION = {
    ...
    'usage': 'Executing <strong>self.run_action("ACTION_ID")</strong> will execute the bash command.'
             'Executing <strong>self.run_action("ACTION_ID", value={"user": "aot", "command": "/home/pi/my_script.sh on"})</strong> will execute the action with the specified command and user.',
...
class ActionModule(AbstractFunctionAction):
    """Execute a Linux bash shell command as a specified user.
    ...
    def run_action(self, dict_vars):
        """Execute the configured bash command and capture output."""
        try:
