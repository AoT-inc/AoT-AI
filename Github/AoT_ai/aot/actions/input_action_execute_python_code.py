...
def generate_code(input_id, python_code):
    """Generate a runnable Python module from user-supplied action code.

    @phase active
    @stability stable
    @dependency PATH_PYTHON_CODE_USER
    """
    error = []
...
class ActionModule(AbstractFunctionAction):
    """Execute user-defined Python 3 code when measurements are acquired.
    ...
    def initialize(self):
        ...
        module_name = "aot.action.{}".format(os.path.basename(file_run).split('.')[0])
    ...
    def run_action(self, dict_vars):
        """Execute the loaded Python code module with measurement data."""
