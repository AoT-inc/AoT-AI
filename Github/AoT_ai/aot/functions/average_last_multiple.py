...
class CustomModule(AbstractFunction):
    """Compute the average of the last measurements from multiple channels.

    Reads the most recent value from each selected measurement channel
    and stores the arithmetic mean to InfluxDB.
    ...
    def initialize(self):
        """Set the initial loop timer with start offset."""
        self.timer_loop = time.time() + self.start_offset

    def loop(self):
        """Collect last measurements from all channels and store their average."""
