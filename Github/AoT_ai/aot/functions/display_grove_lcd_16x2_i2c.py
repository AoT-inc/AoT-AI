...
class CustomModule(AbstractFunction):
    """Display measurement values, timestamps, text, and IP addresses on a Grove 16x2 RGB LCD via I2C.

    Cycles through configurable line sets every period, displaying selected measurements,
    current time, static text, or IP address on each line. Supports RGB backlight color
    control and on/off actions.
    ...
    def initialize(self):
        from aot.devices.lcd_grove_lcd_rgb import LCD_Grove_LCD_RGB
