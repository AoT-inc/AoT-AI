...
def execute_at_creation(error, new_func, dict_functions=None):
    """Create default function channels for a new 16x2 LCD instance."""
    try:
...
def execute_at_modification(
    ...
    """Adjust line-set channels when the user modifies 16x2 LCD settings."""
    page_refresh = False
...
FUNCTION_INFORMATION = {
    ...
    'message': '이 함수는 I2C를 통해 16x2 LCD 디스플레이에 출력을 제공합니다. 이 디스플레이는 한 번에 2줄을 표시할 수 있으므로, 라인 세트 수(Number of Line Sets)가 변경되면 2개 채널씩 추가됩니다. 설정된 주기(Period)마다 LCD가 새로고침되며, 다음 세트의 라인이 표시됩니다. 따라서 처음 표시되는 2줄은 채널 0과 1이며, 이후 2와 3, 그다음 4와 5가 표시되는 방식으로 진행됩니다. 모든 채널이 표시된 후에는 다시 처음부터 순환됩니다.',
...
class CustomModule(AbstractFunction):
    """Control a generic 16x2 I2C LCD to display measurement data.
    ...
    def initialize(self):
        """Initialize the I2C LCD device and configure line sets."""
    ...
    def loop(self):
        """Refresh the LCD display and cycle through line sets."""
    ...
    def output_lcd(self):
        """Generate and write measurement lines to the LCD."""
    ...
    def stop_function(self):
        """Display deactivation message and reset the LCD."""
        self.lcd.lcd_init()
        self.lcd_is_on = True
        self.lcd.lcd_write_lines(
