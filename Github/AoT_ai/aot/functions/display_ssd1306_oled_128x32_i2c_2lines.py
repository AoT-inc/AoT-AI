...
FUNCTION_INFORMATION = {
    ...
    'message': '이 기능은 I2C를 통해 128x32 SSD1306 OLED 디스플레이에 출력을 제공합니다. 이 디스플레이 기능은 한 번에 2줄을 표시할 수 있으므로, 라인 세트 수(Number of Line Sets)가 변경되면 2개 채널씩 추가됩니다. 설정된 주기(Period)마다 LCD가 새로고침되며, 다음 세트의 라인이 표시됩니다. 따라서 처음 표시되는 라인 세트는 채널 0 - 1이며, 이후 2 - 3, 그다음 4 - 5가 표시되는 방식으로 진행됩니다. 모든 채널이 표시된 후에는 다시 처음부터 순환됩니다.',
...
class CustomModule(AbstractFunction):
    """Display measurement values, timestamps, text, and IP addresses on a 128x32 SSD1306 OLED (2-line mode) via I2C.

    Cycles through configurable line sets every period, displaying selected measurements,
    current time, static text, or IP address on each of 2 lines. Supports custom fonts
    and per-line decimal precision settings.

    @phase display
    @stability stable
    @dependency AbstractFunction, DaemonControl, adafruit-circuitpython-ssd1306, adafruit-circuitpython-framebuf, Adafruit-extended-bus, Pillow
    """
    ...
    def initialize(self):
        from aot.devices.lcd_pioled_circuitpython import PiOLEDCircuitpython
    ...
    def stop_function(self):
        self.device.lcd_write_lines(
            "AoT {}".format(AOT_VERSION), "Display Deactivated", "", "")
