...
class CustomModule(AbstractFunction):
    """Implement bang-bang hysteretic control with raise, lower, or both outputs.

    Extends the basic bang-bang pattern to support separate raise and lower
    on/off output channels for bidirectional setpoint control.
    ...
    def initialize(self):
        """Resolve raise and lower output channels and log configuration."""
    ...
    def loop(self):
        """Evaluate measurement and toggle raise/lower outputs per direction."""
    ...
    def stop_function(self):
        """Turn off both raise and lower outputs on deactivation."""
        self.control.output_off(
            self.output_raise_device_id, output_channel=self.output_raise_channel)
        self.control.output_off(
            self.output_lower_device_id, output_channel=self.output_lower_channel)
