...
class Dashboard(CRUDMixin, db.Model):
    """
    Represents a named dashboard page that contains Widgets.

    Dashboards are user-visible pages in the AoT UI. Each dashboard has a name,
    an optional lock flag, and a sort order for sequencing multiple dashboards.
...
class Widget(CRUDMixin, db.Model):
    """
    Represents a single widget element on a Dashboard Tab.

    Widgets render charts, gauges, camera feeds, PID info, or raw values.
    Each widget is positioned on a GridStack canvas via position_x, position_y,
    width, and height. Custom options are stored as JSON.
