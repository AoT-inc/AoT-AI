...
class Camera(CRUDMixin, db.Model):
    """
    Represents a camera device on a Tab.

    Supports USB, CSI, and IP camera types. Handles still capture, timelapse,
    video, and live streaming modes. Configurable via capture_settings JSON and
    standard picture-taking parameters (brightness, contrast, white balance, etc.).
    ...
    camera_type = db.Column(db.Text, nullable=False, default='usb')  # usb, csi, ip
    ...
    auth_username = db.Column(db.Text, default='')
    auth_password = db.Column(db.Text, default='')
    ...
    # Modernization: Capture Modes
    capture_mode = db.Column(db.Text, default='snapshot')  # snapshot, timelapse, video, stream
    capture_settings = db.Column(db.JSON, default={})
