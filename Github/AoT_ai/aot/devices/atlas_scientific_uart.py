# coding=utf-8
"""Communicate with Atlas Scientific sensors over UART serial interface.
...
class AtlasScientificUART(AbstractBaseAtlasScientific):
    """Communicate with Atlas Scientific sensors over UART serial bus.
    ...
    def read_line(self):
        """Read a single line from the UART device using EZO line separator."""
        lsl = len('\r')
        line_buffer = []
        while True:
    ...
    def read_lines(self):
        """Read all available lines from the UART device."""
        lines = []
        try:
            while True:
    ...
    def write(self, cmd):
        """Write a command string by delegating to send_cmd."""
        self.send_cmd(cmd)

    def send_cmd(self, cmd):
        """Send a carriage-return-terminated command to the Atlas sensor."""
...
def main():
    """Run interactive CLI for Atlas Scientific UART device communication.

    @phase active
    @stability stable
    @dependency AtlasScientificUART
    """
