...
if __name__ == "__main__":
    for output_id, output_data in parse_output_information(exclude_custom=True).items():
        ...
        if ('output_manufacturer' in output_data and
                output_data['output_manufacturer'] in ['Linux', 'AoT', 'Raspberry Pi', 'System']):

            if name_str in aot_info and 'dependencies_module' in aot_info[name_str]:
                # Multiple sets of dependencies, append library
                aot_info[name_str]['dependencies_module'].append(output_data['dependencies_module'])
            else:
                # Only one set of dependencies
                aot_info[name_str] = output_data
                if 'dependencies_module' in output_data:
                    aot_info[name_str]['dependencies_module'] = [output_data['dependencies_module']]  # turn into list
        else:
    ...
    list_outputs = [
        (aot_info, "Built-In Outputs (System)"),
