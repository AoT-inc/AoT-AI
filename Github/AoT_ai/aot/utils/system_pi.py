...
logger = logging.getLogger("aot.system_pi")
...
def time_between_range(start_time, end_time, tz=None):
    """
    Check if the current time is between start_time and end_time.

    :param tz: Optional IANA timezone string (e.g. 'Asia/Seoul').  When
               provided, "now" is evaluated in that timezone so that
               HH:MM values entered by the user are compared against
               their local clock, not the server's UTC clock.
    ...
    try:
        ...
        # Resolve "now" in the device's local timezone when available.
        if tz:
            try:
                import pytz
                local_tz = pytz.timezone(str(tz))
                now_time = datetime.datetime.now(local_tz).time().replace(second=0, microsecond=0)
            except Exception:
        ...
        logger.error(f"Error in time_between_range({start_time}, {end_time}, tz={tz}): {e}")
        return 0
    return 0  # No now not within range


def epoch_of_next_time(time_str, tz=None):
    """
    Take time string (HH:MM:SS) and return the epoch of the time in the future.

    :param tz: Optional IANA timezone string.  When provided, the target
               time is interpreted as a wall-clock time in that timezone,
               not in UTC / system-local time.  This ensures a user who
               sets "08:00" gets 08:00 in their local timezone.
    """
    try:
        if tz:
            import pytz
