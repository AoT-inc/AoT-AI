...
from aot.config import (CAMERA_INFO, DEPENDENCIES_GENERAL, FUNCTION_INFO,
...
def custom_options_return_json(
    ...
    if 'custom_options' in dict_options[device]:
        for each_option in dict_options[device]['custom_options']:
            ...
            # [Fix] If request_form is None but use_defaults is True, set default values
            if not request_form:
                if use_defaults and 'default_value' in each_option:
                    ...
                    elif each_option['type'] in ['text', 'multiline_text', 'select', 'select_custom_choices',
                                                   'select_measurement', 'select_measurement_from_this_input',
                                                   'select_channel', 'select_measurement_channel',
                        ...
                        if value is not None:
                            ...
                            if is_auth_field and isinstance(value, str) and len(value) > 5:
                                try:
                                    ...
                                    # Use the new common utility
                                    utils_settings.auto_register_api_key(
                                        value=value,
                                        name=f"{display_name} {each_option['name']} (Auto-saved)",
                                        provider=provider,
                                        tag=each_option['id'],
                                        url=primary_url,
                                        description=desc
                                    ...
                                    logger.error(f"Failed to auto-save auth field via utility: {e}")
...
def dashboard_widget_get_info(dashboard_id=None):
    ...
    if dashboard_id:
        ...
        widgets = Widget.query.filter(Widget.tab_id == dashboard_id).all()
    else:
...
def test_sql():
    try:
        ...
        PID.query.delete()
        db.session.commit()
        logger.error("SQL uuid 생성 테스트 시작: "
                     "{n}개의 항목...".format(n=num_entries))
        ...
        logger.error("완료. 전체: {tot}개. "
                     "평균 실행 시간: {avg:.2f} 초, "
                     "평균 삽입 속도: {epm:.2f} 개/분".format(
                        tot=PID.query.count(),
                        avg=avg_run_time,
                        epm=(factor_info / avg_run_time) * 60.0))
    except Exception as msg:
        logger.error("항목 생성 중 오류 발생: {err}".format(err=msg))
                    ...
                    if key in options:
                        if options[key]['type'] == 'integer':
                            try:
                                ...
                                logger.error("옵션 '{}'의 값이 정수가 아닙니다: '{}'".format(key, value))
                        elif options[key]['type'] == 'float':
                            try:
                                args_dict[key] = float(value)
                            except:
                                logger.error("옵션 '{}'의 값이 실수가 아닙니다: '{}'".format(key, value))
                        elif options[key]['type'] == 'bool':
                            try:
                                args_dict[key] = bool(value)
                            except:
                                logger.error("옵션 '{}'의 값이 Bool 값이 아닙니다: '{}'".format(key, value))
