  ...
  `;
    ...
    class AotMapSearchFixed extends HTMLElement {
        ...
        selectLocation(item) {
            ...
            // Normalize: backend returns lat/lng; fallback to lat/lon (Nominatim raw)
            const lat = parseFloat(item.lat ?? item.latitude);
            const lng = parseFloat(item.lng ?? item.lon ?? item.longitude);

            if (isNaN(lat) || isNaN(lng)) {
                console.error('[AoTSearch] selectLocation: invalid coordinates in item:', JSON.stringify(item));
