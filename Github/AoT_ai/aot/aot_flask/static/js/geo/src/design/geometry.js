...
class AoTGeoGeometry {
    ...
    updateMeasurementLabels(layer) {
        ...
        // Hide labels while panning/zooming to reduce DOM pressure; restore when idle
        if (this.parent.map) {
            const map = this.parent.map;
            map.off('movestart', layer._measurementLabelMoveHandler);
            map.off('moveend',   layer._measurementLabelIdleHandler);
            layer._measurementLabelMoveHandler = () => { if (layer._map) hideLabels(); };
            layer._measurementLabelIdleHandler = () => { if (layer._map) showLabels(); };
            map.on('movestart', layer._measurementLabelMoveHandler);
            map.on('moveend',   layer._measurementLabelIdleHandler);
        }
    ...
    createConnectionDot(lat, lng, typeCode, color) {
        ...
        // z-order is guaranteed by 'connectionPane'; bringToFront RAF removed for performance.
