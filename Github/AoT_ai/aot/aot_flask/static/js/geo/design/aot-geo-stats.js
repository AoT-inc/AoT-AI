        ...
        try {
            ...
            const collectFromGroup = (group) => {
                ...
                group.eachLayer(l => {
                    const props = l.feature?.properties;
                    if (!props) return;
                    // Use node_id if available; fall back to db_id for old data without node_id
                    const id = props.node_id || ('db:' + props.db_id);
            ...
            allSites.forEach(siteLayer => {
                ...
                // Total layers for this site = site-level + all layers in embedded zones (deduplicated)
                const _siteSeen = new Set();
                const _addToSite = (layers) => {
                    layers.forEach(l => {
                        const id = l.feature?.properties?.node_id;
                        if (id) {
                            if (_siteSeen.has(id)) return; // skip duplicates
                            _siteSeen.add(id);
                        }
                        siteTotalLayers.push(l);
                    });
                };
                let siteTotalLayers = [];
                _addToSite(layersBySite[sid] || []);

                zonesInSite.forEach(z => {
                    const zid = z.feature.properties.node_id;
                    _addToSite(layersByZone[zid] || []);
                    _addToSite(layersBySite[zid] || []);
                });
                ...
                // [Fix] Clear _force_include so zone stats use correct fullLength attribution
                siteTotalLayers.forEach(l => { if (l.feature?.properties) delete l.feature.properties._force_include; });
                ...
                zonesInSite.forEach(zoneLayer => {
                    ...
                    // Aggregate layers from both buckets, deduplicating by node_id
                    const _zRaw = (layersByZone[zid] || []).concat(layersBySite[zid] || []);
                    const _zSeen = new Set();
                    const zoneLayers = _zRaw.filter(l => {
                        const id = l.feature?.properties?.node_id;
                        if (!id) return true; // no node_id → always include
                        if (_zSeen.has(id)) return false;
                        _zSeen.add(id);
                        return true;
                    });
                    
                    console.log(`[DesignStats DEBUG] Zone ${zid}: zoneLayers=${zoneLayers.length} (sprinklers in layersByZone=${(layersByZone[zid]||[]).length}, pipes in layersBySite=${(layersBySite[zid]||[]).length})`);
                    const zoneStats = window.AoTMapUtils.calculatePolygonStats(zonePoly, zoneLayers);
                    console.log(`[DesignStats DEBUG] Zone ${zid} stats: pipes=${zoneStats.pipeDetails.length} (main=${zoneStats.pipeMainCount}, branch=${zoneStats.pipeBranchCount}), sprinklers=${zoneStats.objects.sprinklers.length}, emitters=${zoneStats.emitters}`);
                    ...
                    // [Fix] Backfill flow for legacy sprinkler_coverage without flow property
                    const zoneFlowDefault = parseFloat(zonePoly.properties?.gen_config_sprinkler?.flow) || 0;
                    if (zoneFlowDefault > 0) {
                        zoneStats.objects.sprinklers.forEach(sp => {
                            if (!sp.properties.flow && sp.properties.sub_type === 'sprinkler_coverage') {
                                sp.properties.flow = zoneFlowDefault;
                            }
                        });
                    }
                ...
                try {
                    ...
                    if (polyList.length > 0) {
                        ...
                        // Iterative union — use FeatureCollection API (turf v6+)
                        for (let i = 1; i < polyList.length; i++) {
                            try {
                                if (!polyList[i]) continue;
                                const fc = window.turf.featureCollection([unionPoly, polyList[i]]);
                                const res = window.turf.union(fc);
                        ...
                        if (unionPoly && unionPoly.geometry && unionPoly.geometry.type) {
            ...
            allSites.forEach(s => {
                if (s.feature && s.feature.geometry && s.feature.geometry.type) {
                    try {
                        globalTotals.area += (window.turf.area(s.feature) || 0);
                    } catch (e) {
                        console.warn('[DesignStats] Area calculation failed for site:', s.feature.properties?.node_id, e);
                    }
                }
            });
            ...
            processedIds.forEach(id => {
                ...
                // Check featureGroup first (layers currently in edit mode live here)
                checkIn(window.AoTMapEditor?.featureGroup);
                ...
                if (type === 'equipment' || type === 'aot_device' || type === 'device') {
                    ...
                    // Devices
                    // sprinkler_coverage is the canonical emitter; sprinkler dot markers are ephemeral
                    else if (subType === 'sprinkler_coverage') {
