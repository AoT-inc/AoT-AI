...
OUTPUT_INFORMATION = {
    ...
    'dependencies_module': [
        ('internal', 'file-exists /opt/AoT/pigpio_installed', 'pigpio'),
...
class OutputModule(AbstractOutput):
    """Drive PWM output via Raspberry Pi GPIO using the pigpio library.
    ...
    def initialize(self):
        """Configure pigpio connection and apply startup duty cycle state."""
        import pigpio
    ...
    def output_switch(self, state, output_type=None, amount=None, output_channel=None):
        """Set PWM duty cycle via hardware or software pigpio method."""
    ...
    def is_on(self, output_channel=None):
        """Return current duty cycle from pigpio or False if off."""
        if self.is_setup():
            try:
    ...
    def duty_cycle_to_pigpio_value(duty_cycle):
        """Convert duty cycle percentage (0-100) to pigpio range (0-1000)."""
    ...
    def pigpio_value_to_duty_cycle(pigpio_value):
        """Convert pigpio range (0-1000) back to duty cycle percentage (0-100)."""
