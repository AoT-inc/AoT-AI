...
class OutputModule(AbstractOutput):
    """Control a bipolar stepper motor via generic driver (DRV8825, A4988, etc.).
    ...
    def initialize(self):
        """Configure GPIO pins, step resolution, and enable pin for the stepper."""
        ...
        if (self.options_channels['pin_mode_1'][0] and
            ...
            self.logger.warning(
                ...
                "on the board and not via AoT GPIO pins).")
    ...
    def output_switch(self, state, output_type=None, amount=None, output_channel=None):
        """Step the motor by the given amount; positive is CW, negative is CCW."""
...
class StepperMotor:
    """Drive a bipolar stepper motor via GPIO step/dir/enable pins.

    @phase active
    @stability stable
    @see https://github.com/dimschlukas/rpi_python_drv8825
    """
    ...
    def run(self, steps, clockwise):
        """Pulse the step pin for the given number of steps in the given direction."""
        if self.dir_pin:
