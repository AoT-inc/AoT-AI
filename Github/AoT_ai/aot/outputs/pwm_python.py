...
def generate_code(code_pwm, unique_id):
    """Generate a Python module file from user-supplied PWM code.

    @phase active
    @stability stable
    @dependency textwrap, system_pi
    """
...
def execute_at_modification(
    ...
    """Evaluate user PWM code with pylint when the output configuration is saved.

    @param messages: dict of info, warning, error, success messages
    @param mod_output: WTForms object containing form data from the web GUI
    @param request_form: custom_options form input data
    @param custom_options_dict_presave: presave custom options dict
    @param custom_options_channels_dict_presave: presave channel options dict
    @param custom_options_dict_postsave: postsave custom options dict
    @param custom_options_channels_dict_postsave: postsave channel options dict
    @return: tuple of (messages, mod_output, custom_options_dict_postsave, custom_options_channels_dict_postsave)
    """
...
class OutputModule(AbstractOutput):
    """Drive PWM output by executing user-supplied Python 3 code.

    @phase active
    @stability stable
    @dependency AbstractOutput, pylint
    """
    ...
    def initialize(self):
        """Load user Python code module and apply startup duty cycle state."""
        ...
        try:
            ...
            module_name = "aot.output.{}".format(os.path.basename(file_run).split('.')[0])
    ...
    def output_switch(self, state, output_type=None, amount=None, output_channel=None):
        """Execute user Python code with the target duty cycle percentage."""
    ...
    def set_duty_cycle(self, args_dict):
        """Handle custom command to set duty cycle from the UI."""
