...
class OutputModule(AbstractOutput):
    """Control a peristaltic pump via Atlas Scientific EZO-PMP over I2C/UART/FTDI.

    @phase active
    @stability stable
    @dependency AbstractOutput, pylibftdi
    """
    ...
    def initialize(self):
        """Set up Atlas Scientific EZO-PMP device on the configured interface."""
    ...
    def record_dispersal(self, amount_ml=None, seconds_to_run=None):
        """Record pump volume and duration measurements to InfluxDB."""
    ...
    def output_switch(self, state, output_type=None, amount=None, output_channel=None):
        """Send EZO-PMP serial commands to dispense, run continuously, or stop."""
    ...
    def is_on(self, output_channel=None):
        """Check dispensing state and last measurement timestamp to determine if pump is on."""
        if self.is_setup():
            if self.currently_dispensing:
                return True
    ...
    def dispense_ml(self, args_dict):
        """Dispense a specified volume for calibration purposes."""
    ...
    def calibrate(self, level, ml):
        """Send calibration or clear-calibration command to the EZO-PMP."""
        try:
            if level == "clear":
                write_cmd = "Cal,clear"
    ...
    def set_i2c_address(self, args_dict):
        """Change the I2C address of the EZO-PMP device."""
