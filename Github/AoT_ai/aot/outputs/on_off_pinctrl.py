...
class OutputModule(AbstractOutput):
    """Switch a Raspberry Pi 5 GPIO pin on/off via pinctrl.
    ...
    def initialize(self):
        """Configure the GPIO pin via pinctrl and apply the startup state."""
    ...
    def output_switch(self, state, output_type=None, amount=None, output_channel=0):
        """Toggle the GPIO pin to the requested on or off state."""
        try:
            if state == 'on':
    ...
    def switch_pin(self, pin, state):
        """Execute a pinctrl set command to drive the pin high or low."""
        if state:
            set_opt = "dh"
        else:
            set_opt = "dl"
